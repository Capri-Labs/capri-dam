...
module Api
  module V1
    class AssetsController < ApplicationController
      ...
      # Maps a `?variant=` query param to the version/asset property keys that
      # hold its storage path and content type. Used by {#serve_local} (local
      # dev/test delivery) to serve alternate renditions — the flattened image
      # preview, and (new) the video poster thumbnail / MP4 / OGG renditions
      # generated by {AssetProcessorWorker} for non-natively-playable video
      # formats — without adding a bespoke endpoint per rendition type.
      VARIANT_PROPERTY_MAP = {
        "preview"      => { path_key: "preview_storage_path", content_type_key: "preview_content_type", default_content_type: "image/png" },
        "video_poster" => { path_key: "video_poster_path", content_type_key: "video_poster_content_type", default_content_type: "image/jpeg" },
        "video_mp4"    => { path_key: "video_mp4_rendition_path", content_type_key: "video_mp4_rendition_content_type", default_content_type: "video/mp4" },
        "video_ogg"    => { path_key: "video_ogg_rendition_path", content_type_key: "video_ogg_rendition_content_type", default_content_type: "video/ogg" },
      ...
      def create
        ...
        if file.respond_to?(:size) && file.size.to_i > max_upload_size_bytes
          render json: {
            error: "File exceeds the maximum upload size of #{(max_upload_size_bytes / 1.gigabyte.to_f).round(2)} GB.",
      ...
      def serve_local
        ...
        variant_config = VARIANT_PROPERTY_MAP[params[:variant].to_s]
        preview_path = resolved_preview_storage_path(asset, selected_version)
        variant_path = variant_config ? resolved_variant_storage_path(asset, selected_version, variant_config) : nil

        storage_path =
          if variant_config && variant_path.present?
            variant_path
          elsif want_preview && preview_path.present?
            preview_path
          else
        ...
        content_type =
          if variant_config && variant_path.present?
            resolved_variant_content_type(asset, selected_version, variant_config)
          else
            resolved_local_content_type(asset, selected_version, want_preview: want_preview, preview_path: preview_path)
          end
      ...
      # @return [Integer] the configured maximum upload size in bytes.
      #   Admin-configurable via Tools › Asset Configurations › Upload Limits
      #   (persisted as the +max_upload_size_bytes+ {Setting}); defaults to
      #   2 GB when unset, matching AEM's documented default file size limit.
      ...
      def resolved_variant_storage_path(asset, version, variant_config)
        key = variant_config[:path_key]
        return version&.properties&.fetch(key, nil) if params[:version_id].present?

        version&.properties&.fetch(key, nil) || asset.properties[key]
      end

      def resolved_variant_content_type(asset, version, variant_config)
        key = variant_config[:content_type_key]
        if params[:version_id].present?
          version&.properties&.fetch(key, nil).presence || variant_config[:default_content_type]
        else
          (version&.properties&.fetch(key, nil) || asset.properties[key]).presence || variant_config[:default_content_type]
        end
      end
      ...
      def format_asset(asset)
        ...
        metadata = asset.properties.merge(active_v&.properties || {})
        {
          ...
          video_mp4_rendition_url: asset_variant_url_for(asset, "video_mp4", path_property_key: "video_mp4_rendition_path"),
          video_ogg_rendition_url: asset_variant_url_for(asset, "video_ogg", path_property_key: "video_ogg_rendition_path"),

...
export default function UploadGrid({
                                       filesData, setFilesData, getRootProps, getInputProps, isDragActive,
                                       handleToggleSelectAll, handleToggleSelectFile, handleRemoveFile,
                                       allSelected, selectedCount, onClose, globalMeta, handleSingleFileAi, onOpenDuplicate
                                   }) {
    return (
                        ...
                        <Grid item xs={12} sm={6} md={4} lg={3} key={fData.id}>
                            <Paper elevation={fData.selected ? 3 : 0} sx={{ bgcolor: '#ffffff', borderRadius: 2, overflow: 'hidden', border: '1px solid', borderColor: fData.isDuplicate ? '#f59e0b' : fData.selected ? '#4f46e5' : '#e2e8f0', transition: 'all 0.2s' }}>
                                ...
                                <Box sx={{ p: 2 }}>
                                    ...
                                    <TextField fullWidth size="small" variant="standard" value={fData.meta.title} onChange={(e) => setFilesData(prev => prev.map(f => f.id === fData.id ? { ...f, meta: { ...f.meta, title: e.target.value } } : f))} InputProps={{ disableUnderline: true, sx: { fontSize: '0.875rem', fontWeight: 700, color: '#1e293b' } }} />
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                        <Select size="small" value={fData.meta.type || globalMeta.imageType} displayEmpty onChange={(e) => setFilesData(prev => prev.map(f => f.id === fData.id ? { ...f, meta: { ...f.meta, type: e.target.value } } : f))} sx={{ flexGrow: 1, fontSize: '0.75rem', height: 32 }}>

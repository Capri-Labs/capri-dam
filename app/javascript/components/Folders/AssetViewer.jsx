...
// (QuickTime, AVI, WMV, MKV, …) needs the MP4 rendition generated by
// AssetProcessorWorker (see video_mp4_rendition_url) to be playable inline.
const NATIVE_PLAYABLE_VIDEO_MIME_TYPES = [ 'video/mp4', 'video/webm', 'video/ogg' ];

// Plain-text document MIME types that render an inline text preview in the
// asset viewer (see AssetProcessorWorker::PLAIN_TEXT_MIME_TYPES, which is the
// backend source of truth this list mirrors).
const PLAIN_TEXT_DOCUMENT_MIME_TYPES = [ 'text/plain', 'text/csv' ];

// Feature-detects AV1 (in a WebM container) playback support. When true, the
// smaller/higher-quality AV1 rendition (see `video_av1_rendition_url`,
// generated by AssetProcessorWorker for every video when FFmpeg has an AV1
// encoder) is preferred over the universal MP4/H.264 compatibility rendition
// — this is the "AV1 as default delivery codec" strategy from the codec
// roadmap. Falls back gracefully (returns false) in non-browser/test
// environments or older browsers lacking AV1 decode support.
function supportsAv1Playback() {
    if (typeof document === 'undefined') return false;
    try {
        const probe = document.createElement('video');
        if (!probe || typeof probe.canPlayType !== 'function') return false;
        const result = probe.canPlayType('video/webm; codecs="av01.0.05M.08"');
...
export default function AssetViewer({ asset: initialAsset, open, onClose, onAssetUpdated }) {
    ...
    // Plain-text/CSV documents render an inline text preview using the
    // truncated `text_preview` AssetProcessorWorker stores on upload — no
    // separate network round-trip for the (potentially large) original file.
    const isPlainTextDocument = PLAIN_TEXT_DOCUMENT_MIME_TYPES.includes(asset.properties?.content_type);
    const isNativelyPlayableVideo = NATIVE_PLAYABLE_VIDEO_MIME_TYPES.includes(asset.properties?.content_type);
    // Prefer the AV1/WebM rendition when the browser can decode it — it's
    // smaller and higher quality than the H.264 MP4 rendition at the same
    // bitrate. Native browser playback works directly for MP4/WebM/Ogg
    // sources; other formats (QuickTime, AVI, WMV, MKV, …) rely on the MP4
    // rendition transcoded by AssetProcessorWorker (only available once
    // FFmpeg is installed).
    const videoSrc = isVideo
        ? ((asset.video_av1_rendition_url && supportsAv1Playback())
            ? asset.video_av1_rendition_url
    ...
    const isNativelyRenderableImage = isImage && isWebRenderableImage(asset.properties?.content_type);
    // A format is only previewable when the browser can decode it directly
    // (JPEG/PNG/GIF/WebP/SVG/AVIF) OR a server-side preview was generated.
    // Formats like RAW (CR2/NEF/ARW/…) *are* `image/*` but browsers cannot
    // render them, and a preview is only generated when ImageMagick has the
    // relevant delegate (RAW support, PSD/TIFF/HEIC decoders, …) — without
    // one, falling back to `isImage` alone would try to inline the
    // un-renderable original and show a broken-image icon instead of the
    // proper "download to view" fallback below.
    ...
    const canEditImage = asset.editable ?? isNativelyRenderableImage;
                        ...
                        videoSrc ? (
                            ...
                            <Box sx={{ textAlign: 'center', maxWidth: 420 }}>
                                <Typography variant="subtitle1" sx={{ color: '#334155', fontWeight: 600, mb: 1 }}>
                                    {translate('assetViewer.video.transcodingRequiredTitle', "Video preview isn't available for this format yet.")}
                                </Typography>
                                <Typography variant="body2" color="textSecondary" sx={{ mb: 2 }}>
                                    {translate('assetViewer.video.transcodingRequiredHint', 'This video needs to be transcoded to a browser-playable format. Download the original to view it, or reprocess the asset once FFmpeg is installed.')}
                                </Typography>
                                <Button variant="outlined" startIcon={<Download />} onClick={handleDownload}>
                                    {translate('assetViewer.video.downloadOriginal', 'Download Original')}
                                </Button>
                            </Box>
                        )
                    ) : isPlainTextDocument ? (
                        asset.properties?.text_preview ? (
                            <Paper
                                variant="outlined"
                                data-testid="asset-viewer-text-preview"
                                sx={{ width: '100%', height: '100%', overflow: 'auto', p: 2, bgcolor: '#ffffff' }}
                            >
                                <Typography
                                    component="pre"
                                    variant="body2"
                                ...
                                {asset.properties?.text_preview_truncated && (
                                    <Typography variant="caption" color="textSecondary" sx={{ display: 'block', mt: 2 }}>
                                        {translate('assetViewer.preview.textPreviewTruncated', 'Preview truncated — download the original to see the full file.')}
                                    </Typography>
                                )}
                            </Paper>
                        ) : (
                            <Box data-testid="asset-preview-download-fallback" sx={{ textAlign: 'center', maxWidth: 420 }}>
                                <Typography variant="subtitle1" sx={{ color: '#334155', fontWeight: 600, mb: 1 }}>
                                    {translate('assetViewer.preview.notAvailableForFileType', 'Preview not available for this file type.')}
                                </Typography>
                                <Typography variant="body2" color="textSecondary" sx={{ mb: 2 }}>
                                    {translate('assetViewer.preview.downloadToViewHint', 'This file format can\'t be displayed inline. Download the original to open it in its native application.')}
                                </Typography>
                                <Button variant="outlined" startIcon={<Download />} onClick={handleDownload}>
                                    {translate('assetViewer.preview.downloadOriginal', 'Download Original')}
                                </Button>
                            </Box>
                        )

...
// Video MIME types every modern browser can play natively. Anything outside
// this list falls back to the MP4 rendition generated by AssetProcessorWorker
// (video_mp4_rendition_url), when FFmpeg has produced one.
...
// Card-view video thumbnail: shows the server-generated poster frame (see
// AssetProcessorWorker#generate_video_poster) with a Play/Pause overlay
// button (per the "Play video preview in Card view only" requirement — List
// view intentionally has no such control). Clicking Play swaps the static
// poster <img> for an inline, muted, looping <video> preview; clicking Pause
// (or the video finishing) swaps back to the poster.
function AssetVideoThumbnail({ posterUrl, videoSrc, displayName, t }) {
  ...
  return (
    <Box sx={{ position: 'relative', height: '100%', width: '100%', bgcolor: '#0f172a' }}>
      {isPlaying && videoSrc ? (
        <Box
          component="video"
          src={videoSrc}
          ...
          data-testid="asset-grid-video-preview-playing"
          sx={{ height: '100%', width: '100%', objectFit: 'cover' }}
        />
      ) : posterUrl ? (
        <Box component="img" src={posterUrl} alt={displayName} loading="lazy"
             sx={{ height: '100%', width: '100%', objectFit: 'cover' }} />
      ) : (
        <Box sx={{ height: '100%', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <VideoFile sx={{ fontSize: 64, color: '#3b82f6' }} />
        </Box>
      )}
      ...
      {videoSrc && (
        <IconButton
          ...
          aria-label={isPlaying
            ? t('folders.grid.pauseVideo', { defaultValue: 'Pause video preview' })
            : t('folders.grid.playVideo', { defaultValue: 'Play video preview' })}
          data-testid="asset-grid-video-play-toggle"
          sx={{
            position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
            // The card's decorative gradient overlay (rendered as a later
            // sibling, `position: absolute; inset: 0`) would otherwise paint
            // on top of this button and swallow its clicks — establish a
            // stacking context above it so Play/Pause stays clickable.
            zIndex: 11,
            bgcolor: 'rgba(15, 23, 42, 0.55)', color: '#fff', '&:hover': { bgcolor: 'rgba(15, 23, 42, 0.75)' }
          }}
        >
  ...
  return (
    ...
    >
      {assets.map((asset) => {
        ...
        // Video Card-view preview: poster thumbnail (see AssetProcessorWorker's
        // FFmpeg-generated video_poster_url) plus an inline play source —
        // native playback for MP4/WebM/Ogg, or the transcoded MP4 rendition
        // for other formats (QuickTime, AVI, WMV, MKV, …) once available.
        const videoPosterUrl = asset.video_poster_url || null;
        const isNativelyPlayableVideo = NATIVE_PLAYABLE_VIDEO_MIME_TYPES.includes(contentType);
        const videoPlaySrc = isVideo
          ? (isNativelyPlayableVideo ? (asset.url || asset.preview_url) : (asset.video_mp4_rendition_url || null))
          : null;
            ...
            >
              {isProcessing ? (
                ...
                <Box sx={{ height: '100%', width: '100%', bgcolor: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {!isPdf && !isAudio && !is3D && <InsertDriveFile sx={{ fontSize: 64, color: '#64748b' }} />}
                </Box>
              )}

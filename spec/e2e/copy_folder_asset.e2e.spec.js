  ...
  const response = await page.request.post('/api/v1/assets', {
    multipart: {
      file: {
        name: 'copy-e2e.txt',
        mimeType: 'text/plain',
        buffer: Buffer.from('Copy e2e fixture content'),
      },
      title,
...
async function getFolderContents(page, folderId) {
  const response = await page.request.get(`/api/v1/folders/${folderId || 'root'}`);
  expect(response.ok()).toBeTruthy();
  return response.json();
}
...
test.describe('Copy folder/asset — Tools menu overlay', () => {
  ...
  test('copies a single selected folder into another folder via Tools > Copy, leaving the original in place', async ({ page }) => {
    const sourceName = uniqueName('E2E Tools Copy Source Folder');
    const source = await createFolderViaApi(page, sourceName);
    createdFolderIds.push(source.id);

    const destinationName = uniqueName('E2E Tools Copy Destination Folder');
    ...
    await page.getByTestId('tools-menu-copy').click();

    const dialog = page.getByRole('dialog').filter({ hasText: /copy 1 item/i });
    ...
    await Promise.all([
      page.waitForResponse((res) => res.url().includes('/api/v1/copy_operations') && res.request().method() === 'POST'),
      dialog.getByRole('button', { name: /copy/i }).click(),
    ]);

    // The original stays exactly where it was — Copy never removes anything.
    await expect(page.getByTestId(`folder-card-${source.id}`)).toBeVisible();

    await page.goto(`/folders?folder=${destination.id}`);
    await page.waitForLoadState('networkidle');

    const destContents = await getFolderContents(page, destination.id);
    const copiedFolder = (destContents.folders || []).find((f) => f.name === sourceName);
    expect(copiedFolder, `expected a copy named "${sourceName}" inside the destination folder`).toBeTruthy();
    createdFolderIds.push(copiedFolder.id);

    const copiedCard = page.getByTestId(`folder-card-${copiedFolder.id}`);
    await waitForCardWithReload(page, copiedCard);
    await expect(copiedCard).toContainText(sourceName);
  });

  test('copies a single selected asset to root via Tools > Copy, leaving the original in its folder', async ({ page }) => {
    const folderName = uniqueName('E2E Tools Copy Asset Folder');
    const folder = await createFolderViaApi(page, folderName);
    createdFolderIds.push(folder.id);

    const assetTitle = uniqueName('E2E Copy Asset');
    ...
    // Original untouched in its source folder.
    await expect(page.getByTestId(`asset-grid-item-${assetId}`)).toBeVisible();

    await page.goto('/folders');
    await page.waitForLoadState('networkidle');

    const rootContents = await getFolderContents(page, 'root');
    const copiedAsset = (rootContents.assets || []).find((a) => a.title === assetTitle);
    expect(copiedAsset, `expected a copy titled "${assetTitle}" at the root`).toBeTruthy();
    createdAssetIds.push(copiedAsset.id);

    const copiedItem = page.getByTestId(`asset-grid-item-${copiedAsset.id}`);
    await waitForCardWithReload(page, copiedItem);
    await expect(copiedItem).toContainText(assetTitle);
  });

  test('keeps the Copy option enabled even when the selection includes an item without delete rights', async ({ page }) => {
    const folderName = uniqueName('E2E Tools Copy No Delete Rights Folder');
    const folder = await createFolderViaApi(page, folderName);
    createdFolderIds.push(folder.id);
    ...
    await page.getByRole('button', { name: /^tools$/i }).click();
    // Unlike Move (gated by can_delete on the source), Copy never removes
    // anything from the source, so it has no equivalent permission gate on
    // the menu item itself — it should always be enabled for a non-empty
    // selection, independent of delete rights.
    await expect(page.getByTestId('tools-menu-copy')).toBeEnabled();
  });
});

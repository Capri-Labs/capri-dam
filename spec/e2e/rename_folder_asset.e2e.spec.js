  ...
  const response = await page.request.post('/api/v1/assets', {
    multipart: {
      file: {
        name: 'rename-e2e.txt',
        mimeType: 'text/plain',
        buffer: Buffer.from('Rename e2e fixture content'),
      },
      title,
...
test.describe('Rename folder/asset — Tools menu overlay', () => {
  ...
  test('renames a single selected folder via Tools > Rename Folder', async ({ page }) => {
    const originalName = uniqueName('E2E Tools Rename Folder');
    const renamedName = uniqueName('E2E Tools Renamed Folder');
    ...
    const dialog = page.getByRole('dialog').filter({ hasText: /rename folder/i });
    ...
    await expect(page.getByTestId(`folder-card-${folder.id}`)).toContainText(renamedName, { timeout: 15_000 });
    await expect(page.getByText(originalName, { exact: true })).toHaveCount(0);
  });

  test('renames a single selected asset via Tools > Rename Asset', async ({ page }) => {
    const folderName = uniqueName('E2E Tools Rename Asset Folder');
    const folder = await createFolderViaApi(page, folderName);
    createdFolderIds.push(folder.id);

    const originalTitle = uniqueName('E2E Rename Asset');
    const renamedTitle = uniqueName('E2E Renamed Asset');
    await uploadAssetViaApi(page, folder.id, originalTitle);
    const assetId = await getAssetGridId(page, folder.id, originalTitle);
    ...
    await page.getByRole('button', { name: /^tools$/i }).click();
    await page.getByRole('menuitem', { name: /rename asset/i }).click();

    const dialog = page.getByRole('dialog').filter({ hasText: /rename asset/i });
    await expect(dialog).toBeVisible();
    const input = dialog.getByLabel(/new name/i);
    ...
    await Promise.all([
      page.waitForResponse((res) => res.url().includes(`/api/v1/assets/${assetId}`) && res.request().method() === 'PATCH'),
      dialog.getByRole('button', { name: /save/i }).click(),
    ]);

    await expect(page.getByTestId(`asset-grid-item-${assetId}`)).toContainText(renamedTitle, { timeout: 15_000 });
  });

  test('hides the Rename option when both a folder and an asset are selected at once', async ({ page }) => {
    const folderName = uniqueName('E2E Tools Multi Select Folder');
    const folder = await createFolderViaApi(page, folderName);
    createdFolderIds.push(folder.id);
    const assetTitle = uniqueName('E2E Tools Multi Select Asset');
    await uploadAssetViaApi(page, null, assetTitle);
    const assetId = await getAssetGridId(page, null, assetTitle);
    createdAssetIds.push(assetId);
    ...
    await page.getByRole('button', { name: /^tools$/i }).click();
    await expect(page.getByRole('menuitem', { name: /rename folder/i })).toHaveCount(0);
    await expect(page.getByRole('menuitem', { name: /rename asset/i })).toHaveCount(0);
  });
});
